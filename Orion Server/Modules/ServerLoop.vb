Module ServerLoop
    ...
    Public Sub CastNpcSkill(ByVal NpcNum As Long, ByVal MapNum As Long, ByVal skillslot As Long)
        Dim skillnum As Long, MPCost As Long
        Dim Vital As Long, DidCast As Boolean
        Dim i As Long
        Dim AoE As Long, range As Long, VitalType As Byte
        Dim increment As Boolean, x As Long, y As Long
        ...
        Select Case SkillCastType
            Case 0 ' self-cast target
                'Select Case Skill(skillnum).Type
                '    Case SKILL_TYPE_HEALHP
                '        SkillPlayer_Effect(Vitals.HP, True, NpcNum, Vital, skillnum)
                '        DidCast = True
                '    Case SKILL_TYPE_HEALMP
                '        SkillPlayer_Effect(Vitals.MP, True, NpcNum, Vital, skillnum)
                '        DidCast = True
                '    Case SKILL_TYPE_WARP
                '        SendAnimation(MapNum, Skill(skillnum).SkillAnim, 0, 0, TARGET_TYPE_PLAYER, NpcNum)
                '        PlayerWarp(NpcNum, Skill(skillnum).Map, Skill(skillnum).x, Skill(skillnum).y)
                '        SendAnimation(GetPlayerMap(NpcNum), Skill(skillnum).SkillAnim, 0, 0, TARGET_TYPE_PLAYER, NpcNum)
                '        DidCast = True
                'End Select
            ...
            Case 2 ' targetted
                ...
                If MapNpc(MapNum).Npc(NpcNum).TargetType = TARGET_TYPE_PLAYER Then
                    x = GetPlayerX(Target)
                    y = GetPlayerY(Target)
                Else
                    x = MapNpc(MapNum).Npc(Target).x
                ...
                Select Case Skill(skillnum).Type
                    ...
                    Case SKILL_TYPE_DAMAGEMP, SKILL_TYPE_HEALMP, SKILL_TYPE_HEALHP
                        ...
                        If TempPlayer(NpcNum).TargetType = TARGET_TYPE_PLAYER Then
                            If Skill(skillnum).Type = SKILL_TYPE_DAMAGEMP Then
